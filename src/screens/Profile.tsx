    ...
    <View style={styles.page}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.profileCard}>
          <View style={styles.profileTopRow}>
            ...
            <View style={{ marginLeft: 12, flex: 1 }}>
              ...
              <Text style={styles.memberSince}>{t('profile.memberSince')} {getMemberSinceYear(user?.created_at)}</Text>
              <View style={styles.activeBadge}>
                <Text style={styles.activeBadgeText}>{user?.status || t('profile.activeMember')} {t('profile.activeMember')}</Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.infoRows}>
            <View style={styles.row}>
              ...
              <Text style={styles.rowValue}>{user?.id || t('home.notAvailable')}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.rowLabel}>{t('profile.bloodGroup')}</Text>
              <Text style={styles.rowValue}>{user?.blood_group || t('home.notAvailable')}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.rowLabel}>{t('profile.dateOfBirth')}</Text>
              <Text style={styles.rowValue}>{formatDate(user?.date_of_birth)}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.rowLabel}>{t('profile.contact')}</Text>
              <Text style={styles.rowValue}>{user?.phone || t('home.notAvailable')}</Text>
            </View>
            <View style={styles.row}>
              <Text style={styles.rowLabel}>{t('profile.email')}</Text>
              <Text style={styles.rowValue}>{user?.email || t('home.notAvailable')}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.medCard}>
          ...
          <View style={styles.medRow}><Text style={styles.medLabel}>{t('profile.allergies')}</Text><Text style={styles.medValue}>Penicillin, Peanuts</Text></View>
          <View style={styles.medRow}><Text style={styles.medLabel}>{t('profile.chronicConditions')}</Text><Text style={styles.medValue}>Mild Asthma</Text></View>
        ...
        <View style={styles.aboutCard}>
          ...
          {[
            { key: 'about', title: t('profile.aboutMHF'), icon: Info, bg: '#EFF6FF', color: '#075985' },
            { key: 'vision', title: t('profile.ourVision'), icon: Eye, bg: '#ECFDF5', color: '#10B981' },
            { key: 'mission', title: t('profile.ourMission'), icon: Target, bg: '#EFF6FF', color: '#2563EB' },
            { key: 'board', title: t('profile.ourBoard'), icon: Users, bg: '#FDF2FE', color: '#7C3AED' },
            { key: 'team', title: t('profile.ourTeam'), icon: UsersThree, bg: '#FFF7ED', color: '#FB923C' },
            { key: 'volunteers', title: t('profile.ourVolunteers'), icon: Heart, bg: '#FEF2F2', color: '#F43F5E' },
            { key: 'contact', title: t('profile.contactInfo'), icon: Phone, bg: '#F0F9FF', color: '#0369A1' },
          ].map((item) => {
