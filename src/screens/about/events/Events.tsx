...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Dimensions, ActivityIndicator, Linking, Image, Modal } from 'react-native';
...
export default function EventsScreen() {
    ...
    const { data: eventsResponse, isLoading, error, refetch } = useGetEventsQuery();
    ...
    const events = (Array.isArray(eventsResponse?.data) ? eventsResponse?.data : []).map((item: any) => {
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                {error && (
                    <View style={styles.errorContainer}>
                        <Text style={styles.errorText}>{t('events.error', 'Failed to load events')}</Text>
                    </View>
                )}

                {!isLoading && !error && events.length === 0 && (
                    <View style={styles.emptyContainer}>
                        <Text style={styles.emptyText}>{t('events.empty', 'No upcoming events')}</Text>
                    </View>
                )}
                    ...
                    return (
                        <View key={e.id ?? e.title} style={styles.card}>
                            ...
                            <View style={styles.rowHeader}>
                                <TouchableOpacity onPress={() => handleMapPress(e.mapLink)} disabled={!e.mapLink}>
                                    {e.mapLink ? (
                                        <Image
                            ...
                            {e.my_registrations && Array.isArray(e.my_registrations) && e.my_registrations.length > 0 ? (
                                <TouchableOpacity
                                    style={[styles.registrationStatusBadge, getRegistrationStatusColor(e.my_registrations[0]?.customer_status)]}
                                    onPress={() => handleRegistrationStatusPress(e.my_registrations[0])}
                                >
                                    <Text style={styles.registrationStatusText}>
                                        {getRegistrationStatusLabel(e.my_registrations[0]?.customer_status)}
                                    </Text>
                                </TouchableOpacity>
                            ) : isActive && (
                                <TouchableOpacity style={styles.registerBtn} onPress={() => navigation.navigate('RegisterForEvent', { event: e })}>
...
const styles = StyleSheet.create({
    ...
    card: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 14, shadowColor: '#000', shadowOpacity: 0.04, shadowRadius: 4, shadowOffset: { width: 0, height: 2 }, elevation: 2, position: 'relative' },
    rowHeader: { flexDirection: 'row' },
    icon: { fontSize: 22, marginRight: 10 },
    mapIcon: { width: 60, height: 60, marginRight: 10, resizeMode: 'contain', borderColor: '#84b3b3', borderWidth: 0.5, borderRadius: 8, shadowColor: '#000', shadowOpacity: 0.4, shadowRadius: 4, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
    titleContainer: { flex: 1, paddingRight: 90 },
    cardTitle: { fontWeight: '700', color: '#0F172A', flexShrink: 1, flexWrap: 'wrap' },
    meta: { color: '#64748B', fontSize: 12 },
    desc: { color: '#334155', fontSize: 13, marginTop: 8 },

    statusBadge: { position: 'absolute', top: 10, right: 10, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10 },
    ...
    registrationStatusBadge: { marginTop: 20, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 10, alignItems: 'center' },
    ...
    registerBtn: { backgroundColor: '#0EA5A4', borderRadius: 10, paddingVertical: 10, alignItems: 'center', marginTop: 20 },
