...
export default function ReliefScreen() {
  ...
  const reliefProgramsSource = Array.isArray(reliefResponse?.data)
    ? reliefResponse?.data
    : Array.isArray((reliefResponse as any)?.programs)
    ? (reliefResponse as any)?.programs
    : Array.isArray((reliefResponse as any)?.data?.programs)
    ? (reliefResponse as any)?.data?.programs
    : Array.isArray((reliefResponse as any)?.data?.relief_programs)
    ? (reliefResponse as any)?.data?.relief_programs
    : Array.isArray((reliefResponse as any)?.relief_programs)
    ? (reliefResponse as any)?.relief_programs
    : [];

  const reliefPrograms = reliefProgramsSource.map((item: any) => {
    const program = item?.relief_welfare ?? item;
    const sourceItemId = item?.id;
    const reliefWelfareId =
      item?.relief_welfare?.id ?? item?.relief_welfare_id ?? program?.id;
    const backendModuleId = sourceItemId ?? reliefWelfareId ?? item?.id;
    ...
    return {
      ...
      items_provided: Array.isArray(program?.items_provided)
        ? program?.items_provided
        : [],
  ...
  const steps = [
    {
      k: 1,
      title: t("relief.steps.formTitle"),
      desc: t("relief.steps.formDesc"),
    },
    {
      k: 2,
      title: t("relief.steps.verifyTitle"),
      desc: t("relief.steps.verifyDesc"),
    },
    {
      k: 3,
      title: t("relief.steps.distributeTitle"),
      desc: t("relief.steps.distributeDesc"),
    },
  ];
  ...
  const handleDonateAmountSubmit = (amount: number) => {
    if (!selectedProgramForDonation) {
      return;
    }

    const campaignPayment = {
      module: "relief" as const,
      moduleId: (() => {
        const sourceItemId = selectedProgramForDonation.sourceItemId;
        const reliefWelfareId = selectedProgramForDonation.reliefWelfareId;

        if (
          sourceItemId !== undefined &&
          sourceItemId !== null &&
          sourceItemId !== reliefWelfareId
        ) {
          return sourceItemId;
        }

        return (
          selectedProgramForDonation.backendModuleId ??
          selectedProgramForDonation.id
        );
      })(),
      moduleTitle: selectedProgramForDonation.title,
    ...
    <View style={styles.programCard}>
      {!!item.status && (
        <View
          style={[
            ...
            getProgramStatusColor(item.status),
          ]}
        >
          <Text style={styles.programStatusText}>
            {getProgramStatusLabel(item.status)}
          </Text>
        </View>
      )}
      <View style={styles.programHeader}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {!!item.locations && (
            <Text style={styles.programLocation}>{item.locations}</Text>
          )}
        </View>
      </View>
      ...
      {!!item.map_url && (
        ...
        >
          <Text style={styles.mapLinkText}>
            {t("relief.viewMap", "View on Map")}
          </Text>
        </TouchableOpacity>
      )}
      {item.my_registrations &&
      Array.isArray(item.my_registrations) &&
      item.my_registrations.length > 0 ? (
        <TouchableOpacity
          style={[
            styles.registrationStatusBadge,
            getStatusColor(item.my_registrations[0]?.status),
          ]}
  ...
  return (
    <View style={styles.page}>
        ...
        {isLoading && (
          <View style={styles.centerContainer}>
            ...
            <Text style={styles.loadingText}>
              {t("relief.loading", "Loading relief Programs...")}
            </Text>
          </View>
        )}
        ...
        {error && (
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>
              {t("relief.error", "Failed to load relief Programs")}
            </Text>
          </View>
        )}
        ...
        {!isLoading && !error && reliefPrograms.length > 0 && (
          <>
            <Text style={styles.sectionTitle}>
              {t("relief.activePrograms", "Active Relief Programs")}
            </Text>
            <FlatList
              data={reliefPrograms}
            ...
            <View key={s.k} style={styles.stepRow}>
              <View style={styles.stepLeft}>
                <View style={styles.stepCircle}>
                  <Text style={styles.stepNumber}>{s.k}</Text>
                </View>
              </View>
          ...
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>
              {selectedRequest?.registration_id
                ? t("relief.registrationStatus", "Registration Status")
                : t("relief.requestDetails", "Relief Request Details")}
            </Text>
            ...
            <View style={styles.modalBody}>
              {selectedRequest?.registration_id && (
                <View style={styles.modalRow}>
                  <Text style={styles.modalLabel}>
                    {t("relief.registrationId", "Registration ID:")}
                  </Text>
                ...
                <View style={styles.modalRow}>
                  ...
                  <View
                    style={[
                      ...
                      getStatusColor(selectedRequest?.customer_status),
                    ]}
                  >
              ...
              {!selectedRequest?.review_notes &&
                selectedRequest?.registration_id && (
                  <View style={styles.modalRow}>
                    <Text style={styles.modalValue}>
                      {t("relief.noReviewNote", "No review note available")}
                    </Text>
                  </View>
                )}
              ...
              {selectedRequest?.items_needed &&
                selectedRequest.items_needed.length > 0 && (
                  <View style={[styles.modalRow, { alignItems: "flex-start" }]}>
                    <Text style={[styles.modalLabel, { marginTop: 0 }]}>
                      Items:
                    </Text>
                    <View style={{ flex: 1 }}>
                      {selectedRequest.items_needed.map(
                        (item: any, idx: number) => (
      ...
      <AmountEntryModal
        visible={!!selectedProgramForDonation}
        title="Donate to Relief Program"
        subtitle={selectedProgramForDonation?.title}
        actionLabel="Continue"
        onClose={() => setSelectedProgramForDonation(null)}
        onSubmit={handleDonateAmountSubmit}
      />
    </View>
  );
}
