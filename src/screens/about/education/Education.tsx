...
export default function EducationScreen() {
    ...
    const { data: trainingResponse, isLoading, error, refetch } = useGetTrainingProgramsQuery();
    ...
    const programs: TrainingProgram[] = trainingResponse?.data?.map((item: any) => ({
    ...
    const benefits = [
        {
            icon: '🎓',
            title: t('education.benefits.subsidized.title', 'Free / Subsidized Training'),
            desc: t('education.benefits.subsidized.desc', 'Quality education at no or minimal cost'),
        },
        {
            icon: '📜',
            title: t('education.benefits.certificate.title', 'Certificate on Completion'),
            desc: t('education.benefits.certificate.desc', '85% attendance required for certificate'),
        },
        {
            icon: '👨‍🏫',
            title: t('education.benefits.trainers.title', 'Expert Trainers'),
            desc: t('education.benefits.trainers.desc', 'Learn from experienced professionals'),
        },
    ];

    // Translate impact data
    const impact = [
        { value: '500+', label: t('education.impact.students', 'Students Trained') },
        ...
        <View style={{ flex: 1 }}>
            <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 32 }}>
                <View style={styles.headerCard}>
                    <Text style={styles.headerTitle}>{t('education.header.title', 'Education & Skill Development')}</Text>
                    <Text style={styles.headerSubtitle}>{t('education.header.subtitle', 'Empower yourself with new skills')}</Text>
                </View>

                <Text style={styles.sectionTitle}>{t('education.programs.title', 'Available Training Programs')}</Text>

                {isLoading && (
                    <View style={styles.loadingContainer}>
                        <ActivityIndicator size="large" color="#2563eb" />
                        <Text style={styles.loadingText}>{t('common.loading', 'Loading programs...')}</Text>
                    </View>
                )}

                {error && (
                    <View style={styles.errorContainer}>
                        <Text style={styles.errorText}>{t('education.errors.loadFailed', 'Failed to load training programs')}</Text>
                    </View>
                )}

                {!isLoading && !error && (!programs || programs.length === 0) && (
                    <View style={styles.emptyContainer}>
                        <Text style={styles.emptyText}>{t('education.errors.noPrograms', 'No training programs available')}</Text>
                    </View>
                )}

                {!isLoading && !error && Array.isArray(programs) && programs.map((p: TrainingProgram, idx: number) => {
                            ...
                            <View style={styles.programHeader}>
                                ...
                                <View style={{ flex: 1 }}>
                                    <View style={styles.programTitleRow}>
                                        <Text style={styles.programTitle}>{p.program_name}</Text>
                                        <View style={[styles.statusBadge, { backgroundColor: statusMeta.bg }]}>
                                            <Text style={[styles.statusBadgeText, { color: statusMeta.text }]}>{p.status}</Text>
                                        </View>
                                    </View>
                                    <View style={styles.programTagsRow}>
                                        <View style={styles.tag}><Text style={styles.tagText}>{p.duration}</Text></View>
                                        <View style={styles.tagOutline}><Text style={styles.tagOutlineText}>{p.total_seat} seats</Text></View>
                                    </View>
                                </View>
                            </View>
                            ...
                            <View style={styles.topicsRow}>
                                {p.topics_covered && typeof p.topics_covered === 'string'
                                    ? p.topics_covered.split(',').map((t: string) => (
                                        <View key={t.trim()} style={styles.topicTag}><Text style={styles.topicTagText}>{t.trim()}</Text></View>
                                    ))
                                    : Array.isArray(p.topics_covered) && p.topics_covered.map((t: string) => (
                                        <View key={t} style={styles.topicTag}><Text style={styles.topicTagText}>{t}</Text></View>
                                    ))
                                }
                            </View>
                            <View style={styles.programActions}>
                                {(p as any).my_registrations && Array.isArray((p as any).my_registrations) && (p as any).my_registrations.length > 0 ? (
                                    <TouchableOpacity
                                        style={[styles.registrationStatusBadge, getRegistrationStatusColor((p as any).my_registrations[0]?.customer_status)]}
                                        onPress={() => handleRegistrationStatusPress((p as any).my_registrations[0])}
                                    >
                                        <Text style={styles.registrationStatusText}>
                                            {getRegistrationStatusLabel((p as any).my_registrations[0]?.customer_status)}
                                        </Text>
                                    </TouchableOpacity>
                                ) : isRegisterEnabled && (
                                    <TouchableOpacity
                                        style={[styles.primaryBtnSmall]}
                                        onPress={() => navigation.navigate('RegisterTraining', { program: p })}
                                    >
                ...
                <View style={styles.benefitsCard}>
                    <Text style={styles.sectionTitle}>{t('education.benefits.title', 'Program Benefits')}</Text>
                    {benefits.map(b => (
                ...
                <View style={styles.impactCard}>
                    <Text style={styles.sectionTitle}>{t('education.impact.title', 'Our Impact')}</Text>
                        ...
                        <View style={styles.modalBody}>
                            <View style={styles.modalRow}>
                                ...
                                >
                                    <Text style={styles.modalStatusText}>
                                        {getRegistrationStatusLabel(selectedReview?.customer_status)}
                                    </Text>
                                </View>
                            </View>
                            {selectedReview?.registered_at && (
                                <View style={styles.modalRow}>
                                    ...
                                    <Text style={styles.modalValue}>{selectedReview.registered_at}</Text>
                                </View>
                            )}
                            {selectedReview?.review_note && (
                                <View style={[styles.modalRow, { alignItems: 'flex-start' }]}>
                                    ...
                                    <Text style={styles.modalValue}>{selectedReview.review_note}</Text>
                                </View>
                            )}
                            {!selectedReview?.review_note && (
                                <View style={styles.modalRow}>
                                    <Text style={styles.modalValue}>No review notes available.</Text>
                                </View>
                            )}
                        </View>
                        <TouchableOpacity
