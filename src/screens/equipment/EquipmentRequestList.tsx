...
export default function EquipmentRequestList() {
    ...
    const { data: requestsResponse, isLoading, error, refetch } = useGetEquipmentRequestsQuery();
    ...
    return (
        <View style={styles.page}>
            <View style={styles.headerCard}>
                <Text style={styles.headerTitle}>{t('equipment.title', 'Equipment Requests')}</Text>
                ...
                >
                    <Text style={styles.headerBtnText}>{t('equipment.newRequest', 'New Request')}</Text>
                </TouchableOpacity>
            </View>
            ...
            {isLoading && (
                <View style={styles.loadingContainer}>
                    ...
                    <Text style={styles.loadingText}>{t('equipment.loading', 'Loading requests...')}</Text>
                </View>
            )}

            {error && (
                <View style={styles.errorContainer}>
                    <Text style={styles.errorText}>{t('equipment.error', 'Failed to load requests')}</Text>
                </View>
            )}

            {!isLoading && !error && requests.length === 0 && (
                <View style={styles.emptyContainer}>
                    <Text style={styles.emptyText}>{t('equipment.empty', 'No equipment requests found')}</Text>
                </View>
            )}
                    ...
                    <View style={styles.modalContent}>
                        ...
                        <View style={styles.modalBody}>
                            <View style={styles.modalRow}>
                                ...
                                <Text style={styles.modalValue}>{selectedReview?.request_id}</Text>
                            </View>
                            <View style={styles.modalRow}>
                                <Text style={styles.modalLabel}>Status:</Text>
                                <View style={[styles.modalStatusBadge, getStatusColor(selectedReview?.status)]}>
                            ...
                            <View style={styles.modalRow}>
                                ...
                                <Text style={styles.modalValue}>{selectedReview?.equipment_type}</Text>
                            </View>
                            <View style={styles.modalRow}>
                                <Text style={styles.modalLabel}>Duration:</Text>
                                <Text style={styles.modalValue}>{selectedReview?.duration}</Text>
                            </View>
                            {selectedReview?.review_notes && (
                                <View style={[styles.modalRow, { alignItems: 'flex-start' }]}>
                                    <Text style={[styles.modalLabel, { marginTop: 0 }]}>Review Notes:</Text>
                                    <Text style={styles.modalValue}>{selectedReview.review_notes}</Text>
                                </View>
                            )}
                            {!selectedReview?.review_notes && (
                                <View style={styles.modalRow}>
...
const styles = StyleSheet.create({
    ...
    detailLabel: { fontSize: 13, color: '#64748B', marginRight: 6, fontWeight: '500' },
    detailValue: { fontSize: 13, color: '#0F172A', fontWeight: '600', marginLeft: 4 },
    reasonContainer: {
    ...
    reasonLabel: { fontSize: 12, color: '#64748B', fontWeight: '600', marginBottom: 4 },
    ...
    modalLabel: { fontWeight: '700', color: '#475569', marginRight: 8, minWidth: 100 },
