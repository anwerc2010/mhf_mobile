...
import { View, Text, StyleSheet, KeyboardAvoidingView, Platform, Alert, ImageBackground, Image, TouchableOpacity } from 'react-native';
import { useTranslation } from 'react-i18next';
import { useForgotPasswordMutation } from '@psi/shared-api';
...
function ForgotPasswordScreen({ navigation }: ForgotPasswordScreenProps) {
  ...
  useEffect(() => {
    ...
    const message =
      'data' in error
        ? (error.data as { message?: string })?.message || t('forgotPassword.errorMessage', 'Failed to send reset instructions. Please try again.')
        : t('forgotPassword.errorMessage', 'Failed to send reset instructions. Please try again.');
  ...
  const handleForgotPassword = async () => {
    if (!email) {
      Alert.alert(t('common.error'), t('forgotPassword.enterEmail', 'Please enter your email address'));
      return;
    }

    if (!isValidEmail(email)) {
      Alert.alert(t('common.error'), t('forgotPassword.invalidEmail', 'Please enter a valid email address'));
      return;
    }
    ...
    try {
      ...
      if (result?.error) {
        const apiMessage =
          result?.message || t('forgotPassword.errorMessage', 'Failed to send reset instructions. Please try again.');
      ...
      Alert.alert(
        ...
        t('forgotPassword.successMessage', 'Password reset instructions have been sent to your email address.'),
        [
          {
            text: t('common.ok'),
      ...
      const message =
        typeof err === 'object' && err && 'data' in (err as any)
          ? ((err as any).data as { message?: string })?.message || t('forgotPassword.errorMessage', 'Failed to send reset instructions. Please try again.')
          : t('forgotPassword.errorMessage', 'Failed to send reset instructions. Please try again.');
          ...
          <View style={styles.content}>
            ...
            <View style={styles.form}>
              ...
              <View style={styles.button}>
                <PTButton
                  title={t('forgotPassword.sendInstructions', 'Send Instructions')}
                  variant='success'
