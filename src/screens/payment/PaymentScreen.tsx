...
function PaymentScreen() {
  ...
  const { customerId, healthCardId, cardType, purpose, existingOrder } =
    route.params;
  ...
  const displayAmount = existingOrder
    ...
    : pricingData?.data?.[cardType]?.[purpose];
  ...
  const handlePay = async () => {
    ...
    if (outcome.success) {
      navigation.replace("PaymentSuccess", { result: outcome.data });
    } else if (!outcome.dismissed) {
      Alert.alert("Payment Failed", outcome.error, [{ text: "OK" }]);
    }
  };

  if (isPricingLoading && !existingOrder) {
    return (
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      >
        {loading ? (
          ...
          <PTText style={styles.payButtonText}>Pay Now</PTText>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
