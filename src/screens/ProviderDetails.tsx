...
export default function ProviderDetailsScreen() {
    ...
    const handleShare = async () => {
        try {
            const message = `Check out ${provider.provider_name}${provider.type ? ` - ${provider.type}` : ''}
...
${provider.map_url ? `\n🗺️ ${provider.map_url}` : ''}`;
    ...
    const handleFavorite = async () => {
        const subject = `Favorite Provider: ${provider.provider_name || 'Provider'}`;
        const body = `I would like to save this provider as a favorite: Provider Name: ${provider.provider_name || 'N/A'} Type: ${provider.type || 'N/A'}
...
${provider.specialities && provider.specialities.length > 0 ? `Specialities: ${provider.specialities.join(', ')}` : ''}
                ...
                <View style={styles.section}>
                    ...
                    <TouchableOpacity style={styles.contactItem} onPress={handleOpenMap}>
                        ...
                        <View style={styles.contactContent}>
                            ...
                            <Text style={styles.contactValue}>{provider.address || 'N/A'}</Text>
                            <Text style={styles.contactSubValue}>
                                {[provider.city, provider.district, provider.state, provider.pincode]
                                    .filter(Boolean)
                                    .join(', ')}
                            </Text>
                        </View>
