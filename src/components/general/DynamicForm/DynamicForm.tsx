...
import { useForm, Controller, useFieldArray, FieldValues, UseFormReturn, RegisterOptions } from 'react-hook-form';
import { Plus, Trash } from 'phosphor-react-native';
import { useTheme } from '../../../hooks/useTheme';
import { PTButton, PTCard, PTDatePicker, PTInput, PTSelect, PTSwitch, PTText } from '../../comman';
...
export interface PTDynamicFormProps {
     ...
     */
    onSelectionChange?: (fieldId: string, selectedValue: any, allValues: Record<string, any>) => void;
    /**
     * Callback when a field loses focus
     */
    onFieldBlur?: (fieldId: string, value: any, allValues: Record<string, any>) => void;
    /**
    ...
    if (field.validations && Array.isArray(field.validations)) {
        for (const validation of field.validations) {
            switch (validation.name) {
                case 'required':
                    if (validation.value) {
                        rules.required = validation.message || `${field.label || getFieldId(field)} is required`;
                    }
                    break;
                ...
                case 'max':
                    rules.max = {
                        value: validation.value,
                        message: validation.message || `Value must not exceed ${validation.value}`,
                    };
                    break;
        ...
        <View style={{ marginBottom: theme.spacing.lg }}>
            {section.title && (
                <View style={{
                    ...
                    <PTText variant="h4" color="text" style={{ fontWeight: '600' }}>
                        {section.title}
                    </PTText>
                    {showAddButton && arrayFields.length < maxItems && (
                        ...
                        >
                            ...
                            <PTText variant="caption" color="textTertiary" style={{ marginLeft: theme.spacing.xs, fontWeight: '600' }}>
                                {addButtonText}
                            </PTText>
                        </TouchableOpacity>
                    )}
                    ...
                    <View style={{
                        ...
                        {showRemoveButton && arrayFields.length > minItems && (
                            ...
                            >
                                ...
                                <PTText variant="caption" color="error" style={{ marginLeft: theme.spacing.xs, fontWeight: '500' }}>
                                    {removeButtonText}
                                </PTText>
                            </TouchableOpacity>
                        )}
...
const PTDynamicForm = React.forwardRef<PTDynamicFormRef, PTDynamicFormProps>(
    (
        ...
        React.useImperativeHandle(
            ref,
            () => ({
                setValue: (path: string, value: any) => setValue(path, value, { shouldValidate: true }),
                getValues,
                validate: () => trigger(),
            ...
            [setValue, getValues, trigger, handleSubmit, onSubmit, reset, watch, setFocus, clearErrors, formState]
        );
        ...
        const renderField = useCallback(
            (field: FormField, fieldPath: string) => {
                ...
                const fieldError = fieldPath.split('.').reduce((acc: any, key) => acc?.[key], errors);
                        ...
                        return (
                            <Controller
                                ...
                                render={({ field: { onChange, value } }) => (
                                    <View style={[{ marginBottom: theme.spacing.md }, field.style]}>
                                        <PTSwitch
                                            value={Boolean(value)}
                                            ...
                                            {field.values?.map((option) => (
                                                <TouchableOpacity
                                                    ...
                                                    style={{
                                                        ...
                                                        borderRadius: theme.borderRadius.md,
                                                        backgroundColor: value === option.id ? theme.colors.primaryLight : theme.colors.backgroundSecondary,
            ...
            [control, errors, handleValueChange, handleFieldBlur, handleFieldFocus, theme]
        );

        // Render object section
        const renderObjectSection = (section: FormSection, sectionIndex: number) => {
            return (
                <View key={section.id || sectionIndex} style={{ marginBottom: theme.spacing.lg }}>
                    {section.title && (
                        <View style={{
                            marginBottom: theme.spacing.md,
                            paddingBottom: theme.spacing.sm,
